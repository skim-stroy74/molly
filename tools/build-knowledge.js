...
out.push('Телефон: +7 (912) 805-37-37.');
...
/* Обе цены пишем цифрами. Раньше «выходит 2250 руб.» стояло в конце фразы
   и относилось к обеим скидкам сразу — помощница честно повторяла, что
   и по карте выйдет 2250, хотя пять процентов от 2500 это 2375.
   Считать она не должна: в справочнике должен лежать готовый ответ. */
out.push('Имениннику и компании от 10 человек скидка на банкетное меню:');
out.push('10% при оплате наличными — 2250 руб. с человека,');
out.push('5% при оплате картой — 2375 руб. с человека.');
...
/* Банкетное меню — прямо со страницы, вместе с выходом блюда: банкеты
   спрашивают чаще всего, и заказчику важно, что входит и сколько граммов. */
const bqOut = [];
const bqBlocks = index.split('<div class="bq-group').slice(1);
for (const блок of bqBlocks) {
  const раздел = (блок.match(/<h3>([^<]+)<\/h3>/) || [])[1];
  if (!раздел) continue;
  const блюда = [];
  const liRe = /<li>([\s\S]*?)<\/li>/g;
  let li;
  while ((li = liRe.exec(блок))) {
    const имя  = (li[1].match(/<span class="bq-name">([\s\S]*?)<\/span>/) || [])[1];
    const вых  = (li[1].match(/<span class="bq-out">([^<]*)<\/span>/) || [])[1];
    const сост = (li[1].match(/<p class="bq-desc">([\s\S]*?)<\/p>/) || [])[1];
    if (!имя) continue;
    блюда.push('- ' + strip(имя) + (вых ? ' — ' + strip(вых) : '') +
               (сост ? '. ' + strip(сост) : ''));
  }
  if (блюда.length) bqOut.push('-- ' + strip(раздел) + ' --', ...блюда);
}
if (bqOut.length) {
  out.push('== ЧТО ВХОДИТ В БАНКЕТНОЕ МЕНЮ ==');
  out.push('Гость выбирает: общие закуски на компанию, закуску на персону,');
  out.push('один салат и одно горячее. Выход блюда указан в граммах.');
  out.push(...bqOut);
  out.push('');
}

/* Новый год — отдельная страница, но разметка меню та же.
   С октября по декабрь это главный вопрос гостей, и помощница должна
   отвечать по датам и ценам, а не отправлять звонить. */
let ng = '';
try { ng = read('novyi-god.html'); } catch (e) { /* страницы может не быть */ }
if (ng) {
  out.push('== НОВОГОДНИЕ ВЕЧЕРИНКИ И МЕНЮ ==');
  out.push('Тема вечеринок: «Там, на неведомых дорожках».');
  out.push('Вечеринки идут с 11 декабря, начало в 20:00.');
  out.push('В стоимость входят и новогоднее меню, и шоу-программа. Считается с персоны.');
  out.push('Цена зависит от даты:');
  out.push('- будние дни декабря — 3000 руб. с персоны;');
  out.push('- 18, 19, 25 и 26 декабря — 3800 руб. с персоны;');
  out.push('- 31 декабря — 5000 руб. с персоны.');
  out.push('Горячее «Филе лосося на гриле» добавляет 300 руб. к стоимости меню на персону.');
  out.push('Свои напитки приносить можно.');
  out.push('В программе: фотозона-терем, народные забавы от ведущих в сказочных костюмах,');
  out.push('сказочные герои с поздравлениями, живой вокал и танцпол.');
  out.push('Ведущий — Данил Рахматуллин, Дед Мороз — Денис Якушев,');
  out.push('вокал — Иван Шуватов, танцпол — DJ Заря, две Снегурочки: Каролина и Алиса.');
  out.push('Подробности и полное меню — на странице «Новый год» сайта.');

  const ngOut = [];
  for (const блок of ng.split('<div class="bq-group').slice(1)) {
    const раздел = (блок.match(/<h3>([^<]+)<\/h3>/) || [])[1];
    if (!раздел) continue;
    const блюда = [];
    const liRe = /<li>([\s\S]*?)<\/li>/g;
    ...
    while ((li = liRe.exec(блок))) {
      ...
      const сост = (li[1].match(/<p class="bq-desc">([\s\S]*?)<\/p>/) || [])[1];
      if (!имя) continue;
      блюда.push('- ' + strip(имя) + (вых && strip(вых) ? ' — ' + strip(вых) : '') +
                 (сост ? '. ' + strip(сост) : ''));
    }
    if (блюда.length) ngOut.push('-- ' + strip(раздел) + ' --', ...блюда);
  }
  if (ngOut.length) out.push(...ngOut);
  out.push('');
}
...
out.push('Способы: по телефону +7 (912) 805-37-37; во ВКонтакте vk.com/molly174;');
...
/* ---------- чего мы не знаем ---------- */
out.push('== ТОЛЬКО ОБ ЭТОМ СВЕДЕНИЙ НЕТ — тут честно говори «не знаю» и отправляй звонить ==');
