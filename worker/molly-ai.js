...
парковка, дресс-код, вместимость зала. Тогда предложи позвонить: +7 (912) 805-37-37.
...
async function сводка(env) {
  ...
  /* Заявки на бронь. Они и складывались сюда, но в сводку не попадали —
     то есть страховка «уведомление проглядели, заявка не пропала» не
     работала: прочитать сохранённое было нечем. Отдаём их вместе со
     сводкой, под тем же ключом: здесь имена и телефоны гостей. */
  const заявкиКлючи = await env.STATS.list({ prefix: 'req:', limit: 200 });
  const заявки = [];
  for (const k of заявкиКлючи.keys) {
    const v = await env.STATS.get(k.name);
    if (v) { try { заявки.push(JSON.parse(v)); } catch (e) {} }
  }
  заявки.sort((a, b) => String(b.d).localeCompare(String(a.d)));
  ...
  return {
    ...
    заявки: заявки,
    заявокНедоставлено: заявки.filter(з => !з.ok).length,
    ...
    if (!проверка.ок) {
      ...
      return reply({
        answer: 'Тут я не возьмусь называть цену по памяти — уточните, пожалуйста, ' +
                'у администратора: +7 (912) 805-37-37. Он скажет точно.',
        engine: 'cloudflare'
      }, 200, head);
    }
