...
(function () {
  ...
  var RULES = [
    ...
    /* про состав и аллергены — раньше меню: иначе на «безглютеновое пиво»
       выпадал обычный список пива, будто оно безглютеновое */
    [/безглютен|глютен|лактоз|аллерг|непереносим|халяль|кошер|состав блюд|из чего сделан/, function () {
      return 'Про состав и аллергены я отвечать не возьмусь — здесь ошибиться нельзя.<br><br>' +
        'Спросите официанта в зале или позвоните: ' + CALL + '. Уточнят на кухне.';
    }],
    [/пиво|пенн|сорта|разлив/,                                         function () { return TOPICS.beer.a(); }],
    [/парковк|машин|припарк/,                                          function () { return dontKnow('parking'); }],
    /* «меню» целиком — только когда о нём и спрашивают: иначе «цезарь в меню»
       показывал бы разделы вместо цены. Границы слова \b в JS кириллицу не видят */
    [/^(покажи |открой |дай |хочу |есть )?мен[юя]$|^что (у вас )?(из )?(еды|поесть|покушать)|какая кухн|кухн[яию]|перекус|проголод|чем кормит/,
      function () { return TOPICS.menu.a(); }],
    [/детск|детьм|детей|с детьми|ребен|ребят|коляск|малыш|подрост|школьник/, function () {
      return 'У нас 18+ — паб с баром и ночной программой, с детьми не пустят.<br><br>' +
        'Если речь о банкете, условия лучше обсудить с администратором: ' + CALL + '.';
    }],
    [/свой торт|свои торт|торт принес|принести торт|со своим тортом|свечи для торта/, function () {
      return 'На банкете — да: свой торт и фрукты принести можно, это в условиях банкетного меню.<br><br>' +
        'Для обычного вечера уточните у администратора: ' + CALL + '.';
    }],
    ...
    /* «сколько стоит вход» прежним правилом не ловилось — порядок слов другой */
    [/(скольк|стоимост|цен[аые]|платн|почем).{0,24}вход|вход.{0,24}(скольк|стоит|платн|цен|бесплат)|вход платн/, function () {
      return 'Вход на вечеринки платный, но назвать сумму я не могу — она зависит от вечера, ' +
        'а придумывать не стану.<br><br>Точную цену скажет администратор: ' + CALL + '.';
    }],
    ...
    [/веган|вегетариан|постн|мясо не ем/, function () {
      /* мясо и рыба отсеиваются отдельно: они попадали в подборку через
         описания вроде «с овощами» — в вегетарианском списке им не место */
      var meat = /сельд|рыб|лосос|тунц|анчоус|креветк|курин|куриц|мясн|говяд|свин|бекон|ветчин|колбас|салями|хамон|утк|индейк|окорок|пепперони|карбонар|болонь/;
      var v = MENU.filter(function (m) {
        return m.side === 'food' && /овощ|греческ|гриб|картоф|сырн|фрукт/.test(m.hay) && !meat.test(m.hay);
      });
  ...
  /* Но не раньше, чем гость разберётся с уведомлением о cookie: баннер
     занимает низ экрана, облачко живёт там же, и вдвоём они наваливаются
     друг на друга. Ждём, пока баннер уйдёт, и здороваемся уже в тишине. */
  function поздороваться() {
    if (!panel.classList.contains('on')) teaser.classList.add('on');
  }

  /* cookie.js сам говорит, когда с ним покончено: по разметке это не
     прочитать — баннер остаётся в теле страницы и после закрытия. */
  function сКукиРазобрались() {
    return document.body.classList.contains('cookie-done');
  }

  setTimeout(function () {
    if (сКукиРазобрались()) { поздороваться(); return; }
    document.addEventListener('molly:cookie', function () {
      setTimeout(поздороваться, 400);
    }, { once: true });
  }, 1200);
})();
