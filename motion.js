...
(function(){
  ...
  /* --- 0в. Первый экран: смена кадров ---
     Одно фото показывает паб в одном состоянии. Несколько показывают
     его как место, где то полный зал, то группа на сцене, то тихое
     дерево с витражами. Правила простые:
     · первый кадр — обычный фон из CSS, скорость первого экрана не
       страдает, показ начинается только после полной загрузки страницы;
     · следующий кадр грузится заранее и появляется, только когда готов —
       иначе мигала бы пустота;
     · смена медленная: это фон, а не анимация, она не должна тянуть
       взгляд с кнопки брони;
     · только на большом экране: на телефоне фон пролистывают за секунду,
       и платить за него полмегабайта трафика незачем;
     · при системной настройке «меньше движения» и в режиме экономии
       трафика показ не включается вовсе. */
  (function(){
    var фон = document.querySelector('.hero-bg[data-slides]');
    if (!фон) return;

    var адреса = фон.getAttribute('data-slides').split('|')
                   .map(function(a){ return a.trim(); })
                   .filter(Boolean);
    if (!адреса.length) return;

    /* Только на большом экране. На телефоне это лишние полмегабайта ради
       фона, который гость пролистывает за секунду, — там остаётся один кадр. */
    var экономия = navigator.connection && navigator.connection.saveData;
    if (reduce || экономия || window.innerWidth < 900) return;

    var ПОКАЗ = 7000;      /* сколько кадр висит */
    var ВСЕГО = адреса.length + 1;   /* нулевой кадр — фон из CSS */
    var слои = [], номер = 0;
    ...
    function погасить(){
      слои.forEach(function(x){ x.classList.remove('on'); });
    }

    function следующий(){
      номер = (номер + 1) % ВСЕГО;

      if (номер === 0) {              /* вернулись к первому кадру */
        погасить();
        setTimeout(следующий, ПОКАЗ);
        return;
      }

      var i = номер - 1;
      var адрес = адреса[i];
      var предзагрузка = new Image();

      предзагрузка.onload = function(){
        var el = слои[i] || (слои[i] = слой());
        el.style.backgroundImage = 'url(' + адрес + ')';
        погасить();
        el.classList.add('on');
        setTimeout(следующий, ПОКАЗ);
      };
      /* не достучались до файла — просто не меняем кадр */
      предзагрузка.onerror = function(){ setTimeout(следующий, ПОКАЗ); };
      предзагрузка.src = адрес;
    }

    function пуск(){ setTimeout(следующий, ПОКАЗ); }
    if (document.readyState === 'complete') пуск();
    else window.addEventListener('load', пуск);
  })();
