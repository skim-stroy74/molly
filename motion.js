...
(function(){
  ...
  (function(){
    ...
    /* На телефоне берём облегчённые копии: полноразмерные кадры весят
       больше полумегабайта на троих, а показываются шириной в ладонь. */
    var узко = window.innerWidth < 900;
    var адреса = фон.getAttribute('data-slides').split('|')
                   .map(function(a){ return a.trim(); })
                   .filter(Boolean)
                   .map(function(a){ return узко ? a.replace(/\.jpg$/, '-m.jpg') : a; });
    if (!адреса.length) return;

    var экономия = navigator.connection && navigator.connection.saveData;
    if (reduce || экономия) return;
